...
Rectangle {
    ...
    visible: valid && tileData.positionInLayout > 0
    ...
    DragHandler {
        ...
        onDragPointChanged: {
            ...
            if (handle.orientation == Qt.Horizontal) {
                tileData.resizeByPixels(dragPoint.x - oldPoint.x, Qt.LeftEdge);
            } else {
                tileData.resizeByPixels(dragPoint.y - oldPoint.y, Qt.TopEdge);
            }
            oldPoint = dragPoint;
        }
    }
